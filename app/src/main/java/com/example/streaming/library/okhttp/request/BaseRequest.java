package com.example.streaming.library.okhttp.request;

import androidx.annotation.NonNull;

import com.example.streaming.library.BaseConstants;
import com.example.streaming.library.okhttp.OkHttpManager;
import com.example.streaming.library.okhttp.cache.CacheEntity;
import com.example.streaming.library.okhttp.cache.CacheManager;
import com.example.streaming.library.okhttp.cache.CacheMode;
import com.example.streaming.library.okhttp.callback.AbsCallback;
import com.example.streaming.library.okhttp.https.HttpsUtils;
import com.example.streaming.library.okhttp.model.HttpHeaders;
import com.example.streaming.library.okhttp.model.HttpParams;
import com.example.streaming.library.okhttp.utils.HeaderParser;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        ...
        OkHttpManager okHttpUtils = OkHttpManager.getInstance();
    ...
    protected String createUrlFromParams(String url, Map<String, List<String>> params) {
        try {
            ...
            sb.append(url);
            if (url.indexOf('&') > 0 || url.indexOf('?') > 0) sb.append(BaseConstants.AND);
            else sb.append(BaseConstants.QUESTION_MARK);
            for (Map.Entry<String, List<String>> urlParams : params.entrySet()) {
                List<String> urlValues = urlParams.getValue();
                for (String value : urlValues) {
                    //对参数进行 utf-8 编码,防止头信息传中文
                    String urlValue = URLEncoder.encode(value, BaseConstants.UTF8);
                    sb.append(urlParams.getKey()).append(BaseConstants.EQUAL).append(urlValue).append(BaseConstants.AND);
                }
            }
...
//                String headerValue = URLEncoder.encode(entry.getValue(), "UTF-8");
    ...
    protected Call generateCall(Request request) {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && certificates == null && userCookies.size() == 0) {
            return OkHttpManager.getInstance().getOkHttpClient().newCall(request);
        } else {
            OkHttpClient.Builder newClientBuilder = OkHttpManager.getInstance().getOkHttpClient().newBuilder();
            ...
            if (userCookies.size() > 0) {
                OkHttpManager.getInstance().getCookieJar().addCookies(userCookies);
            }
